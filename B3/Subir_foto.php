...
function subirFoto() {
    ...
    error_log("DEBUG Subir_foto: Intentando copiar de '$archivo_temporal' a '$ruta_completa'");
    error_log("DEBUG Subir_foto: Carpeta writable: " . (is_writable($carpeta) ? 'SI' : 'NO'));
    error_log("DEBUG Subir_foto: Archivo temporal existe: " . (file_exists($archivo_temporal) ? 'SI' : 'NO'));
    
    // Intentar copiar el archivo
    if (copy($archivo_temporal, $ruta_completa)) {
        error_log("DEBUG Subir_foto: Copy exitoso");
        // Verificar que el archivo realmente se creó
        if (file_exists($ruta_completa)) {
            error_log("DEBUG Subir_foto: Archivo verificado en $ruta_completa");
            return array('success' => true, 'nombre' => $nuevo_nombre, 'mensaje' => 'Imagen subida correctamente a ' . $ruta_completa);
        } else {
            error_log("DEBUG Subir_foto: Copy exitoso pero archivo no existe");
            return array('success' => false, 'nombre' => '', 'mensaje' => 'Copy retornó true pero el archivo no existe en ' . $ruta_completa);
        }
    } else {
        $error = error_get_last();
        error_log("DEBUG Subir_foto: Copy falló - " . ($error ? $error['message'] : 'sin error'));
